...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // The report was already generated by a real Opus call and saved above; if
  // the charge itself fails (balance already drained), don't strand the user
  // with a report they already paid for and already exists in the DB — same
  // pattern as Task 8's fix (e3f1a2d) and Task 9's fix (4913292).
  try {
    await applyCreditDelta(interview.userId, -CREDIT_COSTS.report_generation, "report_generation", interviewId);
  } catch (err) {
    if (err instanceof InsufficientCreditsError) {
      console.warn(`[interview ${interviewId}] report charge failed (insufficient balance): ${err.message}`);
    } else {
      throw err;
    }
  }
